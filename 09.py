...
# -----------------------
def calculate_moving_average(data, window=4):
...
if selected_com == "Potato":
    ...
    # Calculate moving averages for all years
    ma_data = []
    for year in ["2020", "2021", "2022", "2023", "2024", "2025"]:
        year_data = plot_data[(plot_data["Year"] == year) & (plot_data["Type"] == "Actual")]
        if not year_data.empty:
            ma = calculate_moving_average(year_data["Price"])
            for i, (week, price) in enumerate(zip(year_data["Week"], ma)):
                ma_data.append([week, price, year, "MA"])
        
        # For 2025, also calculate MA for predicted values
        if year == "2025":
            pred_data = plot_data[(plot_data["Year"] == year) & (plot_data["Type"] == "Predicted")]
            if not pred_data.empty:
                # Combine last actual value with predicted values for MA calculation
                combined_prices = [year_data["Price"].iloc[-1]] + pred_data["Price"].tolist()
                ma_pred = calculate_moving_average(pd.Series(combined_prices)).iloc[1:]  # Skip first value (already in actual)
                for i, (week, price) in enumerate(zip(pred_data["Week"], ma_pred)):
                    ma_data.append([week, price, year, "MA_Predicted"])

    ma_df = pd.DataFrame(ma_data, columns=["Week", "Price", "Year", "Type"])

    # Base years
    fig_full = px.line(
        ma_df[(ma_df["Type"] == "MA") & (ma_df["Year"] != "2025")],
        x="Week", y="Price", color="Year",
        title=f"4-Week Moving Average of Spot Price – Potato (2020–2025) - Flood Impact: {flood_impact_factor*100:.1f}%, Post-Flood: {post_flood_impact_factor*100:.1f}%",
        template=custom_template,
        labels={"Week": "Weeks", "Price": "Price"},
        color_discrete_sequence=px.colors.qualitative.Bold
    )

    # --- Actual 2025 MA solid ---
    actual_25_ma = ma_df[(ma_df["Year"] == "2025") & (ma_df["Type"] == "MA")]
    fig_full.add_trace(go.Scatter(
        x=actual_25_ma["Week"],
        y=actual_25_ma["Price"],
        mode="lines",
        name="2025 Actual (MA)",
    ...
    # --- Predicted 2025 MA dashed (connected) ---
    pred_25_ma = ma_df[(ma_df["Year"] == "2025") & (ma_df["Type"] == "MA_Predicted")]
    if not actual_25_ma.empty and not pred_25_ma.empty:
        fig_full.add_trace(go.Scatter(
            x=[actual_25_ma["Week"].iloc[-1]] + pred_25_ma["Week"].tolist(),
            y=[actual_25_ma["Price"].iloc[-1]] + pred_25_ma["Price"].tolist(),
            mode="lines",
            name="2025 Prediction (MA, Flood Adjusted)",
    ...
    if num_weeks >= 2:
        ...
        for yr in ["2020","2021","2022","2023","2024"]:
            if f"spot_price_{yr}" in cdf:
                prices = cdf[f"spot_price_{yr}"].values[:52]
                for week, price in enumerate(prices, 1):
                    all_data.append([week, price, yr, "Actual"])
        
        # 2025 actual
        for week, price in enumerate(df_2025["spot_price_2025"], 1):
            all_data.append([week, price, "2025", "Actual"])
        
        # 2025 predicted
        for i, price in enumerate(predicted_prices, num_weeks + 1):
            all_data.append([i, price, "2025", "Predicted"])
        ...
        # --- Build Figure with Moving Averages ---
        fig = go.Figure()
        colors = ["blue", "red", "green", "purple", "orange"]
        
        # Historical years MA
        for yr, color in zip(["2020","2021","2022","2023","2024"], colors):
            year_ma = ma_df[(ma_df["Year"] == yr) & (ma_df["Type"] == "MA")]
            if not year_ma.empty:
        ...
        # Layout
        fig.update_layout(
            title=f"4-Week Moving Average of {selected_com} Spot Price (2020–2025) - Flood Impact: {flood_impact_factor*100:.1f}%, Post-Flood: {post_flood_impact_factor*100:.1f}%",
            xaxis=dict(
                title="Weeks", range=[1, 52],
                tickmode="linear", dtick=1,
                title_font=dict(size=14, color="black", family="Arial"),
